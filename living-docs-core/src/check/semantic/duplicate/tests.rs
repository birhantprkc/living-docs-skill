...
fn two_distinct_records_are_not_flagged() {
    let a = adr("0001", "the storage backend is config-selected and mutually exclusive between file and database");
    let b = adr("0002", "mermaid validation runs in process via a pure rust parser rather than a docker shell out");
...
fn shared_madr_headings_alone_do_not_make_records_duplicates() {
    let a = adr("0001", "we chose sqlite for the local embedded read model because it needs no server");
    let b = adr("0002", "we chose axum for the web front because it reuses the one rust domain crate");
    assert!(advisories(&[(&a.0, &a.1), (&b.0, &b.1)]).is_empty(), "boilerplate must be stripped");
}
