...
pub fn run(store: &dyn DocStore, docs_dir: &Path, number: &str, description: &str) -> ExitCode {
    match describe(store, docs_dir, number, description) {
        Ok(()) => ExitCode::SUCCESS,
        Err(message) => {
...
/// 3717), exactly as `status.rs` does. Unlike `status`, no vocabulary
/// constrains `description` — any sentence is accepted (issue 0021, part 2
/// of 2) — so the only failure mode is an unresolvable `number`, surfaced by
/// `find_record` before any write is attempted. `description` is quoted via
/// [`format_scalar`] exactly as `new.rs`'s `fill_frontmatter_description`
/// quotes it, so a colon-bearing sentence stays a canonical-check fixed
/// point.
fn describe(
    store: &dyn DocStore,
    docs_dir: &Path,
