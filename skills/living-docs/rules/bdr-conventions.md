...
- **Scenarios** — numbered Given/When/Then statements written to convert verbatim into the project's behavioral regression suite. Each scenario's `Proves` line cites the PRD requirement IDs (`FR-N` / `NFR-N`) it covers (ADR 0035).
...
2. **Scenarios are written to convert verbatim into the project's behavioral regression suite.** Use the exact wording a test author would need. Avoid implementation detail; describe what an external observer would verify. Cite the PRD requirement IDs a scenario proves (`Proves: FR-N`) — `living-docs check` traces each ID a non-Draft PRD defines to a BDR that links that PRD and cites the ID (ADR 0035); a BDR that cites an ID without linking the PRD earns no coverage.
