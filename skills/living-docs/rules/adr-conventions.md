...
10. **Materiality — a decision earns an ADR only when it is expensive to reverse (ADR 0052).** The trail is gated by *materiality*, not by *layer*. A decision gets its own ADR when it changes a stated invariant or constitution article, a public contract or schema, a dependency direction or module boundary, or a pinned external dependency — or it reverses a prior ADR. A decision that is cheap to reverse lives in a `## Decision` section of the issue that carries the work, not in an ADR. The test promotes rule 4 from rejections to decisions: **"would a future reader pay for rediscovering this?"** This is what `strict` enforces (`enforcement-modes.md` trigger 7): a *material* decision shipped without its ADR is a blocked task; a non-material one recorded in its issue is compliant in every mode.
