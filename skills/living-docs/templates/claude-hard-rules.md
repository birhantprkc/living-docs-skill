...
- A record whose body opens with a `> **SUPERSEDED …**` or `> **DEPRECATED …**` callout is history. Follow the successor link, or discard the record if it has none; never plan on it. Read `living-docs effective` for what is in force. The callout is CLI-written by `supersede`, `set`, or `fmt` — never by hand.
